...
public class RedisDemo {
	...
	private static void init() {
		...
		config.setMaxWait(5000); // 获取连接是的最大等待时间，如果超时就抛出异常
		config.setTestOnBorrow(false);// 在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
		config.setTestOnReturn(true);
		jedisPool = new JedisPool(config, "192.168.21.130", 6379, 10000, "", 0); // 配置、ip、端口、连接超时时间、密码、数据库编号（0~15）
	}
	...
	private static void zset() {
		...
		map.put(24.3,"wch"); // 这里以小组成员的年龄来演示
		map.put( 30.0,"lida");
		map.put(23.5,"chf");
		map.put(22.1,"lxl" );
		map.put(24.3,"wch" ); // 这个不会被加入，应该重复了
	...
	private static void zsets() {
		...
		map1.put(24.3,"wch"); // 这里以小组成员的年龄来演示
		map1.put(30.0,"lida" );
		map1.put(23.5,"chf" );
		map1.put(22.1,"lxl" );
